...
OUTPUT_DIR <- file.path(ROOT, "figures", "phasing", "phasing_ed_b_whatshap_ng50_depth")
dir.create(OUTPUT_DIR, recursive = TRUE, showWarnings = FALSE)

FIGURE_STEM <- "phasing_ed_b_whatshap_ng50_depth"
WIDTH_MM <- 88
HEIGHT_MM <- 48
DPI <- 600
...
required <- c(
  ...
  "Phased SNV rate", "Phase blocks", "WhatsHap block NG50",
  "Median block (kb)", "Mean block (kb)", "Max block (Mb)", "Status", "Tool"
)
...
plot_data <- raw |>
  transmute(
    ...
    phased_snv_rate = as.numeric(`Phased SNV rate`),
    phase_blocks = as.numeric(`Phase blocks`),
    ng50_mb = as.numeric(`WhatsHap block NG50`),
    median_block_kb = as.numeric(`Median block (kb)`),
    mean_block_kb = as.numeric(`Mean block (kb)`),
    max_block_mb = as.numeric(`Max block (Mb)`),
    status = Status,
    tool = Tool
  ) |>
  mutate(
...
write_csv(
  plot_data |>
    transmute(
      ...
      source_whatshap_block_ng50_mb = ng50_mb,
      plotted_phase_block_ng50_mb = ng50_mb,
      het_snvs, phased_snvs, phased_snv_rate, phase_blocks,
      median_block_kb, mean_block_kb, max_block_mb,
      transformation = "none; source field is recorded in Mb"
    ),
...
audit <- data.frame(
  ...
  reuse_level = "structural adaptation of the revised LongPhase NG50 depth panel",
  stringsAsFactors = FALSE
)
...
metric_definition <- data.frame(
  ...
  source_field = "WhatsHap block NG50",
  plotted_unit = "Mb",
  transformation = "none; source field is recorded in Mb",
  definition = paste(
...
make_ng50_subpanel <- function(mapper_name, panel_index) {
  ...
  ggplot(panel_data, aes(x = depth_index, y = ng50_mb, colour = platform, group = platform)) +
    ...
    scale_y_continuous(name = if (panel_index == 1L) "Phase-block NG50 (Mb)" else "\u00A0", breaks = seq(0, 4, by = 1), labels = label_number(accuracy = 0.1, trim = FALSE), limits = c(0, 4.25), expand = expansion(mult = 0)) +
    labs(title = mapper_name) + theme_ng50_subpanel()
}
...
message("Rendered WhatsHap NG50 depth trajectories to: ", OUTPUT_DIR)
