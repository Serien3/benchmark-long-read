...
ROOT <- find_root()
BLOCK_FILE <- file.path(ROOT, "data", "phasing_block_stats_longphase.csv")
ACCURACY_FILE <- file.path(ROOT, "data", "phasing_accuracy_longphase.csv")
OUTPUT_DIR <- file.path(ROOT, "figures", "phasing", "phasing_main_b_longphase_30x_profile")
dir.create(OUTPUT_DIR, recursive = TRUE, showWarnings = FALSE)

FIGURE_STEM <- "phasing_main_b_longphase_30x_profile"
WIDTH_MM <- 183
HEIGHT_MM <- 52
DPI <- 600
TARGET_DEPTH <- "30x"
...
required_block <- c(
  ...
  "Phased SNP rate", "NG50 (kb)", "Status"
)
required_accuracy <- c(
  "Dataset", "Reference", "Mapper", "Depth", "Covered variants", "Assessed pairs",
  "Switch errors", "Switch error rate", "Blockwise Hamming", "Hamming rate", "Status"
)
...
if (length(missing_block) > 0L) {
  stop("Missing block-stat columns: ", paste(missing_block, collapse = ", "))
}
if (length(missing_accuracy) > 0L) {
  stop("Missing accuracy columns: ", paste(missing_accuracy, collapse = ", "))
}

normalize_platform <- function(x) sub("_latest$", "", x)
...
if (nrow(block_raw) != 18L || nrow(accuracy_raw) != 18L) {
  stop("Expected 18 rows in each LongPhase source table")
}
if (nrow(block_30x) != 6L || nrow(accuracy_30x) != 6L) {
  stop("Expected six 30x rows in each LongPhase source table")
}
...
wide <- inner_join(
  ...
  mutate(
    Platform = normalize_platform(Dataset),
    phased_benchmark_snps_pct = 100 * `Phased SNPs` / `Benchmark SNPs`,
    phase_block_NG50_Mb = `NG50 (kb)` / 1000,
...
if (nrow(wide) != 6L) stop("The 30x block and accuracy tables did not join one-to-one")
...
if (any(wide$Status.block != "completed") || any(wide$Status.accuracy != "completed")) {
  stop("All plotted rows must have completed status")
}
...
rate_tolerance <- 6e-5
if (max(abs(wide$`Phased SNP rate` - wide$phased_benchmark_snps_pct / 100)) > rate_tolerance) {
  stop("Stored phased SNP rates disagree with recomputed numerator/denominator rates")
}
if (max(abs(wide$`Switch error rate` - wide$switch_errors_per_10000 / 10000)) > 6e-7) {
  stop("Stored switch error rates disagree with recomputed numerator/denominator rates")
}
if (max(abs(wide$`Hamming rate` - wide$blockwise_hamming_error_pct / 100)) > rate_tolerance) {
  stop("Stored Hamming rates disagree with recomputed numerator/denominator rates")
}

metric_spec <- data.frame(
  metric_key = c(
    "phased_benchmark_snps_pct", "phase_block_NG50_Mb",
  ...
  numerator_field = c("Phased SNPs", NA, "Switch errors", "Blockwise Hamming"),
  denominator_field = c("Benchmark SNPs", NA, "Assessed pairs", "Covered variants"),
  transformation = c(
    "100 * Phased SNPs / Benchmark SNPs",
    "NG50 (kb) / 1000",
...
long <- wide |>
  select(
    ...
    `Benchmark SNPs`, `Phased SNPs`, `Phased SNP rate`, `NG50 (kb)`,
    `Assessed pairs`, `Switch errors`, `Switch error rate`,
    `Covered variants`, `Blockwise Hamming`, `Hamming rate`,
...
if (any(long$display_value < 0)) stop("All plotted metrics must be non-negative")

source_data <- long |>
  mutate(
    numerator = case_when(
      metric_key == "phased_benchmark_snps_pct" ~ as.numeric(`Phased SNPs`),
    ...
    denominator = case_when(
      metric_key == "phased_benchmark_snps_pct" ~ as.numeric(`Benchmark SNPs`),
    ...
    source_value = case_when(
      metric_key == "phase_block_NG50_Mb" ~ as.numeric(`NG50 (kb)`),
      metric_key == "phased_benchmark_snps_pct" ~ as.numeric(`Phased SNP rate`),
    ...
    source_value_unit = if_else(metric_key == "phase_block_NG50_Mb", "kb", "fraction")
  ) |>
  transmute(
    ...
    numerator, denominator, source_value, source_value_unit, transformation
  )
...
audit <- data.frame(
  ...
  exclusion_reason = "10x and 50x belong to the separately specified depth-response panels",
  aggregation = "none",
  averaging = "none",
  ...
  reuse_level = "structural adaptation of the supplied four-panel bar-chart reference",
  stringsAsFactors = FALSE
)
...
axis_spec <- tibble::tibble(
  ...
  y_min = c(98.5, 0.0, 0.0, 0.0),
  y_max = c(100.0, 2.0, 1.8, 4.0),
  y_breaks = list(
    c(98.5, 99.0, 99.5, 100.0),
    c(0.0, 0.5, 1.0, 1.5, 2.0),
    c(0.0, 0.5, 1.0, 1.5),
    c(0, 1, 2, 3, 4)
  ),
  label_accuracy = c(0.1, 0.1, 0.1, 1.0),
  baseline_policy = c(
    "truncated 98.5-100% window; explicit ticks; comparisons are within-panel only",
    "zero baseline",
    "zero baseline",
    "zero baseline"
  )
...
make_bar_panel <- function(metric_name) {
  ...
  ggplot(
    panel_data,
    aes(x = bar_x, y = display_value, fill = Platform)
  ) +
    geom_vline(
...
draw_header <- function() {
  grid::grid.text(
    "b", x = grid::unit(1.5, "mm"), y = grid::unit(0.56, "npc"),
    just = c("left", "centre"),
    gp = grid::gpar(
  ...
  grid::grid.text(
    "LongPhase  ·  HG002  ·  GRCh38  ·  matched 30×",
...
message("Rendered LongPhase 30x profile to: ", OUTPUT_DIR)
