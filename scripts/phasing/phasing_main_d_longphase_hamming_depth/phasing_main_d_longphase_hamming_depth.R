...
ROOT <- find_root()
DATA_FILE <- file.path(ROOT, "data", "phasing_accuracy_longphase.csv")
OUTPUT_DIR <- file.path(ROOT, "figures", "phasing", "phasing_main_d_longphase_hamming_depth")
dir.create(OUTPUT_DIR, recursive = TRUE, showWarnings = FALSE)

FIGURE_STEM <- "phasing_main_d_longphase_hamming_depth"
WIDTH_MM <- 88
HEIGHT_MM <- 48
DPI <- 600
...
required <- c(
  ...
  "Blockwise Hamming", "Hamming rate", "Status"
)
missing_columns <- setdiff(required, names(raw))
if (length(missing_columns) > 0L) {
  stop("Missing required columns: ", paste(missing_columns, collapse = ", "))
}
if (nrow(raw) != 18L) stop("Expected exactly 18 LongPhase accuracy rows")
...
numeric_fields <- c("depth_x", "covered_variants", "blockwise_hamming_errors", "hamming_error_pct")
...
metric_definition <- data.frame(
  ...
  interpretation = "Fraction of covered truth variants assigned to the inconsistent haplotype after block orientation correction.",
  stringsAsFactors = FALSE
)
...
make_hamming_subpanel <- function(mapper_name, panel_index) {
  ...
  ggplot(
    panel_data,
    aes(x = depth_index, y = hamming_error_pct, colour = platform, group = platform)
  ) +
    geom_vline(
    ...
    scale_y_log10(
      name = if (panel_index == 1L) "Blockwise Hamming error (%)" else "\u00A0",
      breaks = LOG_BREAKS,
...
message("Rendered LongPhase Hamming depth trajectories to: ", OUTPUT_DIR)
